---
title: 'STA304 Final Project: Causal Inference Analysis between the Factor Immunization Coverage and Life Expectancy'
...
Logistic regression model on treatment is defined as $$log(\frac{\hat{p}}{1-\hat{p}}) = \hat{\beta_0} + \hat{\beta_1} *x_{statusDeveloping} +\hat{\beta_2}*x_{adult\_mortality} +\hat{\beta_3}*x_{Alcohol}+\hat{\beta_4}*x_{total_expenditure} $$
...
knitr::kable(cleaned_data_matched[1:10, 8:16])
```
...
The final multiple linear regression model is defined as:
$$\hat{life\_expectancy} = \hat{\beta_0} + \hat{\beta_1} *x_{statusDeveloping} +\hat{\beta_2}*x_{adult\_mortality} +\hat{\beta_3}*x_{Alcohol}+\hat{\beta_4}*x_{total_expenditure} + $$
$$+ \hat{\beta_5}*x_{GDP} + \hat{\beta_6}*x_{BMI} 
+ \hat{\beta_6}*x_{Schooling} + \hat{\beta_7}*x_{health\_coverage\_percent} + \hat{\beta_8}*x_{vaccine\_coverage\_over\_ave}$$

Coefficients are extracted from the summary table of the model:
...
Other statistics and criteria of the model:
...
Since the propensity score matching process minimizes the effect of possible confounding variables.
